...
export default class Soldado extends EntidadDeCombate {
    ...
    furiaDeAtaque(objetivo) {
        ...
        const danoTotal = (this.getDanoBase() + this.#inventario.getBonoDeDano()) * 2.5;
        objetivo.recibirDano(danoTotal);
        this.#inventario.desgastarCuchillas(3);
        console.log(`> ¡Un golpe devastador inflige ${danoTotal} de daño!`);
        return true;
    }
    ...
    recibirDano(monto) {
        ...
        if (this.#estaDefendiendo) {
            danoFinal *= 0.5;
            console.log(`> ¡La defensa reduce el daño a ${Math.round(danoFinal)}!`);
        }
    ...
    atacar(objetivo) {
        ...
        console.log(`> Le inflige ${danoTotal} de daño. Vida restante de ${objetivo.getNombre()}: ${objetivo.getVida()}`);
    }
    ...
    getExpSiguienteNivel() { return this.#expSiguienteNivel; }
    getModificadorDefensa() { return this.#modificadorDefensa; }
    getEnergia() { return this.#energia; }
    getEnergiaMaxima() { return this.#energiaMaxima; }
    getPuntos() { return this.#puntos; }

    aumentarDefensa(cantidad) { this.#modificadorDefensa += cantidad; }
}
