export default class Inventario {
  ...
  usarPocion(nombrePocion, usuario, objetivo) {
    // Ahora acepta usuario y objetivo
    const index = this.#pociones.findIndex(
      (p) => p.getNombre() === nombrePocion,
    );
    if (index > -1) {
  ...
  getBonoDeDano() {
    return this.#cuchillasActuales
      ? this.#cuchillasActuales.getBonoDeDano()
      : 0;
  }
