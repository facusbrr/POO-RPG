...
export default class CLI {
  ...
  static mostrarEstado(jugador, titanes) {
    ...
    console.log(
      `👤 ${jugador.getNombre()} [Nv. ${jugador.getNivel()}] | ❤️ ${jugador.getVida()}/${jugador.getVidaMaxima()} | 🔥 ${jugador.getEnergia()}/${jugador.getEnergiaMaxima()} | 🛡️ ${(jugador.getModificadorDefensa() * 100).toFixed(0)}% Def.`,
    );

    console.log(
      `   EXP: ${jugador.getExp()}/${jugador.getExpSiguienteNivel()}`,
    );
    console.log(
      `   🗡️ ${cuchillas.getNombre()} (Daño: ${cuchillas.getFilo()}, Dur: ${cuchillas.getDurabilidad()}/${cuchillas.getDurabilidadMaxima()})`,
    );
    console.log("----------------------------------------");
    console.log("ENEMIGOS:");
    titanes.forEach((titan, index) => {
      console.log(
        `  ${index + 1}. ${titan.getNombre()} (${titan.getTipo()}) | ❤️ ${titan.getVida()}/${titan.getVidaMaxima()}`,
      );
    });
    console.log("========================================");
  }
  ...
  static async mostrarMenuCombate(jugador) {
    const cuchillasRotas =
      jugador.getInventario().getCuchillas().getDurabilidad() <= 0;
    const choices = [
      {
        name: "Atacar",
        value: "atacar",
        disabled: cuchillasRotas ? "Cuchillas rotas" : false,
      },
  ...
  static async mostrarMenuMercado(jugador) {
    ...
    const { accion } = await inquirer.prompt({
      ...
      choices: [
        ...
        {
          name: "Mejorar Filo de Cuchillas (25 Puntos)",
          value: "mejorar_cuchillas",
        },
        { name: "Reparar Cuchillas (10 Puntos)", value: "reparar_cuchillas" },
        {
          name: "Reforzar Equipo (+5% Defensa) (30 Puntos)",
          value: "reforzar_equipo",
        },
